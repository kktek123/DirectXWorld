...
void CubeMapDemo::Initialize()
{
	...
	shader = new Shader(L"40_CubeMap.fxo");
	
	cubeMap[0] = new TextureCube(shader, 256, 256);
	cubeMap[1] = new TextureCube(shader, 256, 256);
...
void CubeMapDemo::Update()
{
	...
	ImGui::InputInt("CubeMap Type", (int *)&type);
	type %= 5;

	static float Amount = 1.0f;
	ImGui::SliderFloat("Amount", &Amount, -2.0f, 2.0f);
	shader->AsScalar("Amount")->SetFloat(Amount);
...
void CubeMapDemo::Pass(UINT mesh, UINT model, UINT anim)
{
...
void CubeMapDemo::AddPointLights()
{
	PointLight light;
	light =
	{
...
void CubeMapDemo::AddSpotLights()
{
	SpotLight light;
	light =
	{
