...
void ViewportDemo::Initialize()
{
	...
	render2D[0] = new Render2D();
	render2D[0]->GetTransform()->Position(D3D::Width() * 0.5f, D3D::Height() * 0.5f, 0);
	render2D[0]->GetTransform()->Scale(D3D::Width(), D3D::Height(), 1);
	render2D[0]->SRV(renderTarget[0]->SRV());
	...
	renderTarget[1] = new RenderTarget((UINT)size, (UINT)size);
	depthStencil[1] = new DepthStencil((UINT)size, (UINT)size);
	viewport[1] = new Viewport(size, size);

	render2D[1] = new Render2D();
	render2D[1]->GetTransform()->Position(125, D3D::Height() - 250 + 125, 0);
	render2D[1]->GetTransform()->Scale(250, 250, 1);
	render2D[1]->SRV(renderTarget[1]->SRV());
...
void ViewportDemo::PreRender()
{
	//Main Render
	{
		...
		viewport[0]->Set(width, height, x, y);
