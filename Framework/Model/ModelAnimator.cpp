...
void ModelAnimator::Stop(UINT instance)
{
	tweenDesc[instance].Curr.Speed = 0;
}
...
void ModelAnimator::EraseTransform(UINT index)
{
...
Matrix ModelAnimator::GetBoneAnimationMatrix(UINT instanceId, UINT boneIndex)
{
	if (csOutput == NULL)
	{
		Matrix m;
	...
	ModelAnimator::TweenDesc* tween = &tweenDesc[instanceId];
	ModelClip* clip = model->ClipByIndex(tween->Curr.Clip);
	float time = 1.0f / clip->FrameRate() / tween->Curr.Speed;

	//손 좌표 구하기
	Matrix handMat;
	{
		//현재 애니메이션
		handMat = GetBoneMatrix(tween->Curr.Clip, tween->Curr.CurrFrame, boneIndex);
		Matrix nextHandMat = GetBoneMatrix(tween->Curr.Clip, tween->Curr.NextFrame, boneIndex);
		Math::LerpMatrix(handMat, handMat, nextHandMat, tween->Curr.Time);

		if (tween->Next.Clip > -1)
		{
			//다음 애니메이션
			Matrix nHandMat = GetBoneMatrix(tween->Next.Clip, tween->Next.CurrFrame, boneIndex);
			Matrix nNextHandMat = GetBoneMatrix(tween->Next.Clip, tween->Next.NextFrame, boneIndex);
			Math::LerpMatrix(nHandMat, nHandMat, nNextHandMat, tween->Next.Time);

			Math::LerpMatrix(handMat, handMat, nHandMat, tween->TweenTime);
		}
	}

	return handMat * transforms[instanceId]->World();
}
