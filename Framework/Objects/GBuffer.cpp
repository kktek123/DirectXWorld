	...
	: shader(shader)
{
	...
	CreateDepthStencilView();//깊이를 읽기만 한다.
	CreateDepthStencilState();//하늘을 그리기위해쓴다 
...
void GBuffer::Render()
{
	...
	//{
	//	depthStencil->SRV(), diffuseRTV->SRV(), specularRTV->SRV(), 
	//	emissiveRTV->SRV(), normalRTV->SRV(), tangentRTV->SRV()
	//};
	sSrvs->SetResourceArray(srvs, 0, 6);


	D3D::GetDC()->IASetVertexBuffers(0, 0, NULL, NULL, NULL);//버텍스 버퍼날림
...
void GBuffer::CalcSpotLights(UINT count)
{
	for (UINT i = 0; i < count; i++)
	{
		...
		bool bUp = (direction.y > 1 - 1e-6f || direction.y < -1 + 1e-6f);//업인지아닌지 판단하기
		//파이(1)보다 크면 축이 뒤집힘
		...
		for (int k = 0; k < 3; k++)//ㄷ다이랙션을 이용한 회전행렬
		{
			R.m[0][k] = right[k];
			R.m[1][k] = up[k];
			R.m[2][k] = direction[k];
...
void GBuffer::CreateDepthStencilView()
{
	...
	desc.Flags = D3D11_DSV_READ_ONLY_DEPTH | D3D11_DSV_READ_ONLY_STENCIL;//깊이와 스탠실을 읽기만 한다.
